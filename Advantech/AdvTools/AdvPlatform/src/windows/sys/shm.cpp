...
SHM_ID ADVPLAT_CALL shmget(key_t key, size_t size, int shmflg) {
	...
	if (hMapObject == NULL) {
		if (shmflg & IPC_CREAT) {
			hMapObject = CreateFileMapping(
				...
				0,                      // size: high 32-bits
				static_cast<int>(size + sizeof(SHMID)),  // size: low 32-bits
...
void * ADVPLAT_CALL shmat(SHM_ID shmid, const void *shmaddr, int shmflg) {
...
int ADVPLAT_CALL shmdt(const void *shmaddr) {
	BOOL fRet;
	if((SHM_ID)shmaddr < sizeof(SHMID)) return -1;
