...
int PrintString(char* buffer, int size, JSONode *json) {
	size_t len = 0;
	if(size > json->len+3) {
		snprintf(buffer,json->len+3,"\"%.*s\"", static_cast<int>(json->len), json->s);
		len = json->len+2;
	} else {
		snprintf(buffer,size,"\"%.*s\"", static_cast<int>(json->len),json->s);
		len = size;
	}
	////ADV_DEBUG("<%s,%d>len = %d\n",__FILE__,__LINE__,len);
...
int PrintNumber(char* buffer, int size, JSONode *json) {
	size_t len = 0;
	if(size > json->len+1) {
		snprintf(buffer,json->len+1,"%.*s", static_cast<int>(json->len),json->s);
		len = json->len;
	} else {
		snprintf(buffer,size,"%.*s", static_cast<int>(json->len),json->s);
		len = size;
...
int PrintObject(char* buffer, int size, JSONode *json, int depth) {
	...
	return static_cast<int>(((size_t)pos - (size_t)buffer));
}
...
int PLString(JSONode *json, int depth) {
	...
	printf("(STRING)[%p]{%.*s}  @%p\n",json, static_cast<int>(json->len), json->s, json->collection);
	return depth;
}
