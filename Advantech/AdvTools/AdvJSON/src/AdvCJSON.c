...
int ADVJSON_CALL JSON_Validator(const char *json) {
	...
	if(*e == '}') return 0;
	else return static_cast<int>(check);
}
...
/////////////////////////////////////////////////////////
static int isInteger(char *str, size_t len) {
	int i = 0;
	for(i = 0 ; i < len ; i++) {
...
void ADVJSON_CALL JSON_Get(JSONode *json, const char *path, char *result, size_t size) {
	if(json == NULL) {
