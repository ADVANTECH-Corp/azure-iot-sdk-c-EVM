...
JSONode *GetNewItem(JSONCollect* collect) {
	...
	if (cspare != NULL) {
		if (Queue_GetAmount(cspare_qh) == 0) {
			...
			tag.n = node;
			node->manager = qh;
			Queue_AddWithKey(qh, &tag, sizeof(JSONTag), (CC_KEY)node);
		}
		else {
...
void DestroyItem(JSONCollect* collect, JSONode *node) {
	...
	JSONCollect *cspare = collect->spare;
	QueueHandler cspare_qh = (QueueHandler)cspare->queue;
	if (Queue_GetWithKey(qh, &tag, (CC_KEY)node) == CON_RESULT_NORMAL) {
		tag.n->collection = cspare;
		tag.n->manager = cspare_qh;
		Queue_AddWithKey(cspare_qh, &tag, sizeof(JSONTag), (CC_KEY)tag.n);
	}
	else {
		ReleaseNodeMem(node);
	}
...
void LinkItem(JSONCollect* collect, JSONode *node) {
	...
	node->manager = qh;
	tag.n = node;
	Queue_AddWithKey(qh, &tag, sizeof(JSONTag), (CC_KEY)tag.n);
	return;
}
