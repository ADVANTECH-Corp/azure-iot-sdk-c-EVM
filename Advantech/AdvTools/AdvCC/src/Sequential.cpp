...
void ADVCC_CALL Seq_InitBlocking(SeqHandler *handler, int maxItems) {
    ...
    *handler = (SeqHandler)AdvMalloc(sizeof(SeqHandle));
    ...
    if(maxItems > 0) {
		...
		Semaphore_SetMax((*handler)->mBlockingSem, 0, 0);
...
SeqElement * ADVCC_CALL Seq_SearchString(SeqHandler handler, char *string) {
	...
	while (elm != NULL) {
		...
		Pos = 0;
		while (1) {
			index = (pointer - (char *)elm->mData)&0xFFFFFFFF;
			range = DataLen - index;
			if (range <= 0) break;
...
/**ThreadSafe**/
SeqElement *__Seq_FindKey(SeqHandler handler, CC_KEY key) {
	if (handler == NULL) return NULL;
	if (handler->mSeqAmount == 0) return NULL;
...
int ADVCC_CALL Seq_PeekWithKey(SeqHandler handler, void *data, CC_KEY key) {
...
/**ThreadSafe**/
int ADVCC_CALL Seq_GetAmountWithKey(SeqHandler handler, CC_KEY key) {
    if(handler == NULL) return 0;
...
/*******************OPERATOR**************************/
/**ThreadSafe**/
int ADVCC_CALL Seq_PushTailWithKey(SeqHandler handler, void *data, int dataSize, CC_KEY key) {
	if (handler == NULL) return CON_RESULT_NO_HANDLER;
...
int ADVCC_CALL Seq_PushTailWithKeyBlocking(SeqHandler handler, void *data, int dataSize, CC_KEY key) {
	if (handler == NULL) return CON_RESULT_NO_HANDLER;
...
/**ThreadSafe**/
int ADVCC_CALL Seq_PushHeadWithKey(SeqHandler handler, void *data, int dataSize, CC_KEY key) {
    if(handler == NULL) return CON_RESULT_NO_HANDLER;
    if(data == NULL || dataSize <= 0) return CON_RESULT_ERROR_SOURCE;
...
int ADVCC_CALL Seq_PushHeadWithKeyBlocking(SeqHandler handler, void *data, int dataSize, CC_KEY key) {
	if (handler == NULL) return CON_RESULT_NO_HANDLER;
	__Seq_Sem_WaitForNewSpace(handler);
...
int ADVCC_CALL Seq_PopWithKey(SeqHandler handler, void *data, CC_KEY key) {
	if (handler == NULL) return CON_RESULT_NO_HANDLER;
	if (data == NULL) return CON_RESULT_ERROR_DESTINATION;
	//printf("<%s,%d> Seq_PopHead handler->mSeqAmount = %d\n",__FILE__,__LINE__,handler->mSeqAmount);
...
int ADVCC_CALL Seq_PopWithKeyBlocking(SeqHandler handler, void *data, CC_KEY key) {
	if (handler == NULL) return CON_RESULT_NO_HANDLER;
	__Seq_Sem_WaitForNewResource(handler);
