...
static void *pthread_checkalive(void *in)
{
 ...
 // uri must be lowercase
  ELSAPI_CODE AdvEls_Write( const char *uri, const char* Content, char *Buffer, int BufSize )
  {
	...
	// Add log
	 if( g_pWAPI_Interface != NULL )
		ret = g_pWAPI_Interface->WAPI_Post((const char*)gElsServerInfo.szAddress, gElsServerInfo.nPort, (const char*)uri_lowercase, (const char*)Content, static_cast<int>(strlen(Content)), Buffer, BufSize, (char*)"", 1 );
