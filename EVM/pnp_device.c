...
#define DEVICEDETAILEDINFORMATION_INDEX 0

#define SENSORSINFORMATION_INDEX 1

#define DEVICEINFO_INDEX 2

#define DEFAULT_SEND_TELEMETRY_INTERVAL_MS 5000

#define DEVICE_CAPABILITY_MODEL_URI "urn:Advantech:EVM:1"
...
int pnp_device_initialize(const char* connectionString, const char* trustedCert)
{
    ...
    if ((interfaceClientHandles[DEVICEDETAILEDINFORMATION_INDEX] = DeviceDetailedInformationInterface_Create(digitalTwinDeviceClientHandle)) == NULL)
    {
        LogError("DeviceDetailedInformationInterface_Create failed");
        return -1;
    }

    if ((interfaceClientHandles[SENSORSINFORMATION_INDEX] = SensorsInformationInterface_Create(digitalTwinDeviceClientHandle)) == NULL)
    {
        LogError("SensorsInformationInterface_Create failed");
        return -1;
    }
    ...
    DeviceDetailedInformationInterface_Property_ReportAll();
...
void pnp_device_run()
{
    ...
    if (nowTick - lastTickSend >= DEFAULT_SEND_TELEMETRY_INTERVAL_MS)
    {
        ...
        SensorsInformationInterface_Telemetry_SendAll();
...
{
    if (interfaceClientHandles[DEVICEDETAILEDINFORMATION_INDEX] != NULL)
    {
        DeviceDetailedInformationInterface_Close(interfaceClientHandles[DEVICEDETAILEDINFORMATION_INDEX]);
    }

    if (interfaceClientHandles[SENSORSINFORMATION_INDEX] != NULL)
    {
        SensorsInformationInterface_Close(interfaceClientHandles[SENSORSINFORMATION_INDEX]);
    }
